...
class AuthService {
    ...
    func register(fullName: String, email: String, password: String) async throws -> User {
        ...
        // 2. Crear el modelo de usuario (las estadísticas inician en 0)
        let newUser = User(
            id: UUID(),
            fullName: fullName,
            email: email,
    ...
    func login(email: String, password: String) async throws -> User {
        ...
        guard let userEntity = userManager.validateUser(email: email, password: password) else {
            throw NSError(domain: "AuthService", code: 401,
                          userInfo: [NSLocalizedDescriptionKey: "Correo o contraseña incorrectos."])
        }

        // 2. Retornar el modelo COMPLETO (incluye foto y estadísticas)
        return userManager.user(from: userEntity)
    }
}
